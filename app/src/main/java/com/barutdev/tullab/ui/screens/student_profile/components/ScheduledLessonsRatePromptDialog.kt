package com.barutdev.tullab.ui.screens.student_profile.components
...
import com.barutdev.tullab.ui.screens.student_profile.ScheduledLessonsScope
 ...
 *
 * Uses [tullabStringResource] (not [stringResource]) so that the correct
...
fun ScheduledLessonsRatePromptDialog(
    ...
    AlertDialog(
        ...
        title = {
            Text(text = tullabStringResource(id = titleRes))
        },
        text = {
            Text(text = tullabStringResource(id = messageRes))
        },
        confirmButton = {
            TextButton(onClick = { onConfirm(true) }) {
                Text(text = tullabStringResource(id = confirmRes))
            }
        },
        dismissButton = {
            TextButton(onClick = { onConfirm(false) }) {
                Text(text = tullabStringResource(id = R.string.rate_change_dialog_keep_original))
            }
        }
    )
}
